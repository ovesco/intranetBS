 ...
 */
class UserDemoteCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('app:user:demote')
            ->setDescription('remove role to user')
            ->addArgument(
                'username',
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if($user instanceof User)
        {
            ...
            {
                $user->removeSelectedRoles($role);
                $userRepo->save($user);
                $output->info($username.' has been demoted to '.$role)->writeln();
            }
            else
            {
